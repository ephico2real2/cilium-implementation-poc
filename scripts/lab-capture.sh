#!/usr/bin/env bash
# lab-capture.sh [out-dir] [spec] — the lab's pages as PNGs with an evidence line each, driven by a JSON spec of pages
# (scripts/capture/lab.json: Grafana's dashboards, the Hubble UI, cf2cnp) through scripts/capture/walk.js. On a Mac or a
...
OUT="${1:-captures}"; SPEC="${2:-scripts/capture/lab.json}"; mkdir -p "$OUT"; CTX="${LAB_STACK_CTX:-kind-poc1}"; PW_VERSION="${PW_VERSION:-1.63.0}"
