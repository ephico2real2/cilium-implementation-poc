...
for c in "$@"; do
  ...
  helm upgrade --install cilium cilium/cilium --version "$CILIUM_VERSION" --namespace kube-system --kube-context "$ctx" \
    -f "cilium/values-$c.yaml" -f cilium/values-ci.yaml ${LAB_FEATURES:+-f cilium/values-ci-features.yaml} --set k8sServiceHost="$host" --set k8sServicePort=6443 --wait --timeout 10m >/dev/null
