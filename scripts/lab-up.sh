...
for c in "$@"; do
  ...
  # The mesh apiserver and its certificates are part of the ONE install too, when both clusters are asked for:
  # the chart's default `clustermesh.apiserver.tls.auto.method=helm` renders the server/admin/local/remote
  # certificates from the cluster's CA at install (the CA copied from the first cluster is reused — _helpers.tpl
  # looks the `cilium-ca` secret up), so nothing is generated by a Job afterwards and `cilium clustermesh status`
  # finds `clustermesh-apiserver-remote-cert` at once. `cilium clustermesh enable` did this as a second Helm
  # upgrade plus a certgen Job, restarted the agents, and its status poll printed "Trying to get secret … by
  # deprecated name" until the Job had run (runs 34784194103–34787222878); this way there is nothing to wait for.
  mesh=""; [ $# -ge 2 ] && mesh="--set clustermesh.useAPIServer=true --set clustermesh.apiserver.service.type=NodePort --set clustermesh.apiserver.tls.auto.method=helm"
  helm upgrade --install cilium cilium/cilium --version "$CILIUM_VERSION" --namespace kube-system --kube-context "$ctx" \
    ...
    --set k8sServiceHost="$host" --set k8sServicePort=6443 --set gatewayAPI.enabled=true --set gatewayAPI.enableAlpn=true $mesh --wait --timeout 10m >/dev/null
...
if [ $# -ge 2 ]; then
  say "ClusterMesh: $1 <-> $2 (the apiservers were installed with Cilium; connect only)"
  for c in "$1" "$2"; do kubectl --context "kind-$c" -n kube-system rollout status deploy/clustermesh-apiserver --timeout=5m; done
