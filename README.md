...
cluster — **measured, not quoted**. Two kind clusters in a ClusterMesh with no kube-proxy and no other CNI, 39 demos
...
| [docs/regression/](docs/regression/README.md) | regression testing in plain words — the two-minute check (`scripts/lab-regression.sh`, fourteen PASS/FAIL rows with the number that decided each), Cilium's own connectivity test read line by line, and the slim `lab-regression` Action with its captures; the 1.20.2 runs pasted |
