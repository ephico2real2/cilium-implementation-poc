...
| 18 | **OBI: zero-code traces across the mesh** | OpenTelemetry eBPF Instrumentation on both clusters, scoped to the four bank deployments: one payment as a 16-span tree spanning poc1 → poc2 → Postgres/Redis, spans from both clusters into the demo 10 collector through a global Service, RED metrics per route in the demo 16 Prometheus, Hubble exemplars filling on their own — and the page's Cilium `bpf.tc.priority` change shown unnecessary with tcx |
