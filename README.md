...
cluster — **measured, not quoted**. Two kind clusters in a ClusterMesh with no kube-proxy and no other CNI, 40 demos
...
|---|---|---|---|
| eBPF datapath, identity-aware | every pod gets a security identity; policy and load balancing are eBPF programs, not iptables chains | demos 01, 03 | [Introduction](https://docs.cilium.io/en/stable/overview/intro/), [eBPF datapath](https://docs.cilium.io/en/stable/network/ebpf/intro/), [Identity-based security](https://docs.cilium.io/en/stable/security/network/identity/), [Hubble](https://docs.cilium.io/en/stable/observability/hubble/setup/) |
...
| Enterprise PKI | a cert-manager root in poc1, a `ClusterIssuer` in both clusters issuing the mesh, relay and Gateway certificates; the root in the OS stores and every namespace | demos 08, 24, 36 | [ClusterMesh certificates](https://docs.cilium.io/en/stable/network/clustermesh/setup/#configure-tls-certificates), [Hubble TLS](https://docs.cilium.io/en/stable/observability/hubble/configuration/tls/), [Gateway API HTTPS](https://docs.cilium.io/en/stable/network/servicemesh/gateway-api/https/) |
...
| 09 | [Wildcard TLS + three route types](demos/09-routes/README.md) | wildcard and exact certificates on one Gateway; `HTTPRoute`, `GRPCRoute`, `TCPRoute` from one image, a Go client that tests all three (how the missing-ALPN gotcha #33 was found); Hubble UI behind the Gateway as `hubble.poc.local`, its data stream measured (Part 10) | [3 captures](demos/09-routes/README.md#evidence) |
...
| 39 | [Fixing Cilium itself: a remote pod's workload name](demos/39-remote-workload-fix/README.md) | cilium/cilium#25676 taken to the code: the three files behind the empty `destination_workload`, the fix in a fork (the workload carried on the CiliumEndpoint → ipcache → Hubble), built on this Mac, rolled on both clusters, measured — the first commit fixed L3/L4 flows and not the Envoy-reported Gateway flow (Hubble has two parsers), the second did, the third closed a review edge; Cilium's own L7-by-Workload dashboard fills for a backend on the other node; OB1 and Codex reviewed; upstream already has a draft (#48563) — the lab's review comment is drafted, not posted | [the dashboard after](demos/39-remote-workload-fix/output/l7-by-workload-remote-backend-after-fix.png), `docs/REVIEW_CILIUM_FIX.md` |
...
| 41 | [The shop platform on the mesh — phase 1: the platform behind the doors](demos/41-shop-mesh-phase1/README.md) | the same platform in both clusters, every stateless Service global with affinity local, one URL, a door per cluster, `X-Served-By` naming the door that answered, policies generated from flows | [`output/transcript.txt`](demos/41-shop-mesh-phase1/output/transcript.txt), `check.sh` |
...
| 17 | [Tetragon runtime security](demos/17-tetragon/README.md) | the attempted runtime-security deployment, with the Docker Desktop kernel and host-proc blockers measured to their exact errors and fixes | [recorded blockers](demos/17-tetragon/README.md) |
...
| **DNS:** every `*.poc.local` name written into `/etc/hosts`, all pointing at the one Gateway address `172.18.255.240` | one wildcard A record `*.poc.local` → the Gateway's address, or external-dns creating records from the HTTPRoutes' hostnames |
| **TLS:** one wildcard certificate on the Gateway from cert-manager and the demo 08 root, the root trusted by hand (the System keychain, or `--cacert docs/root-ca.crt` on a curl) | the same root distributed by the platform (demo 36 shows the mechanism), or for a public zone an ACME issuer with DNS-01 — the only ACME path that issues a wildcard; the listener's `certificateRefs` do not change |
...
| the Docker VM kernel (6.6.12-linuxkit — netkit needs 6.7 —, no `CONFIG_SECURITY`, no `CONFIG_NETKIT`) and its memory ceiling: **Tetragon**, **OBI's generic tracer**, **netkit**, the **bandwidth manager with BBR** and **BIG TCP** cannot run (gotchas #60, #66, demo 06 Part 4) | the kernel and the RAM you chose; none of those gotchas apply |
| one VM for seven kind nodes, two Cilium installs and the whole observability stack: control-plane restarts under load, the hub Prometheus OOM-killed, petclinic and poc3 kept scaled down or paused (`scripts/cluster-resume.sh poc3` brings it back; gotchas #66, #77) | real nodes with their own memory |
| the Docker VM's DNS upstream unreachable from pods: CoreDNS forwards to `1.1.1.1` / `8.8.8.8` (gotcha #63) | the site resolvers |
| no second node for the poc2 spoke: the observer's two replicas land on the same worker and the PDB cannot help | anti-affinity that spreads |
...
| open-source Hubble UI: no time range, no flows-per-minute chart, no cluster picker | the same — this lab builds the flow store with Loki instead (demo 25) |
...
| [docs/upstream/](docs/upstream/README.md) | what cilium/cilium is and how it takes changes (their guide, quoted), and each change this lab has ready for it with the problem, the data path, the capture, the fix, screenshots and the exact steps — first the Hubble L7 dashboard for Gateway traffic, then the agent image rescanned (128 → 13 HIGH from 1.20.1 to 1.20.2, and where the 13 belong); §5 the release reports — each upstream release read against what the lab runs (`.claude/skills/upstream-release-notes`) |
| [docs/regression/](docs/regression/README.md) | regression testing in plain words — the two-minute check (`scripts/lab-regression.sh`, fifteen PASS/FAIL rows with the number that decided each), Cilium's own connectivity test read line by line, and the slim `lab-regression` Action with its captures; the 1.20.2 runs pasted |
