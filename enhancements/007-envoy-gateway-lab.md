...
both clusters, the three-command install, the lab root) — **phase 2 done** (demo 51: kube-vip alone, the doors, R7,
the VIP move, gRPC) — **one-cluster lab** (demo 54: `eg-poc1`, kube-vip, HTTP and gRPC through
two Gateways, reachable from the MacBook) — **one-cluster lab, MetalLB** (demo 52: `eg-poc2`, MetalLB L2, the sample app, the gRPC test matrix).
...
| `172.19.255.64/26` | **eg-poc1** (the one-cluster lab, demo 54) | services `.72–.79`; gateways `.100–.110` — HTTP door `.100`, gRPC door `.101` | eg-poc1 (ARP, one node per address) |
| `172.19.255.128/26` | **eg-poc2** (the one-cluster lab, demo 52) | services `.136–.143` (autoAssign); doors `.150–.160` (autoAssign: false) — HTTP door `.150`, gRPC door `.151`. Same `/26` the two-cluster lab gave eg2; `eg-up.sh` refuses the pair, they never coexist | eg-poc2 (ARP, one node per address) |
...
`poc4` 10.40/10.41): **eg1 `10.50.0.0/16` / `10.51.0.0/16`, eg2 `10.60.0.0/16` / `10.61.0.0/16`, eg-poc1 `10.70.0.0/16` / `10.71.0.0/16`, eg-poc2 `10.80.0.0/16` / `10.81.0.0/16`** — so the two labs
...
| 2 — **demo A: kube-vip** — **done 2026-09-18** | 51 | kube-vip DS + cloud-provider in both clusters, ranges per namespace from §3.1; `eg1-gw`/`eg2-gw` with an `EnvoyProxy` each (class kube-vip + `kube-vip.io/loadbalancerIPs`); the R7 experiment recorded (`externalIPs`, 0 ARP); the shared VIP `.16` Gateway created in eg1 only, moved to eg2 and back (`scripts/eg-vip-move.sh kube-vip eg2`), gap 10.486 s / 8.976 s; `shopapi` behind the doors, `X-Served-By`; **gRPC (R10)** `SERVING` h2c + TLS on every door, no BackendTrafficPolicy; `check.sh` 39 PASS | `demos/51-eg-kube-vip/` (`apply.sh`, `check.sh`, `cleanup.sh`, `hosts-entries.sh`, `scripts/eg-vip-move.sh`) | R4, R7, R8, R10 |
| 2b — **one-cluster lab: kube-vip + two Gateways** — **done 2026-09-19** | 54 | one kind cluster `eg-poc1` (kindnet + kube-proxy iptables, no Cilium); two doors that isolate HTTP from gRPC (`http-gw` `.100`, `grpc-gw` `.101`); kube-vip announces both from `eg-poc1-worker` (`fa:1f:d6:0f:1e:ae`, both `/32` on eth0); the Mac's curl (`200` / `X-Served-By: eg-poc1`), grpcurl (`SERVING`) and Chrome (`output/browser.png`); `check.sh` 15 PASS. No fail cases, no MetalLB, no VIP move | `clusters/eg-poc1.yaml`, `demos/54-eg-poc1-kube-vip/` (`apply.sh`, `check.sh`, `cleanup.sh`, `hosts-entries.sh`); `scripts/eg-up.sh eg-poc1` | R2, R4, R8, R10 |
| 2c — **kube-vip in BGP mode (migration)** | 56 | eg-poc1 moves from L2 to BGP on the demo 46 fabric. Pointer: [enhancement 006 §9 / §4 row 56](006-bgp-tutorial.md). Doors `10.98.0.10` / `.11`; demo 54's `.100`/`.101` stop answering; cleanup restores `clusters/eg/kube-vip-ds.yaml` | `demos/56-kube-vip-bgp/` | R4, R8, R10 |
| 3 — **demo B: MetalLB** | 52 | MetalLB alone on eg-poc2, the sample app, the gRPC test matrix. The earlier idea of MetalLB beside kube-vip on eg1/eg2 (issue #57) is superseded by the operator's design. | `demos/52-eg-poc2-metallb/` | R5, R8, R10 |
...
| D8 | A self-signed root per lab, minted on that lab's `ROOT_HOME` and exported by `scripts/eg-up.sh`, never committed (issue #60): the two-cluster lab's `.tmp/eg-root-ca.crt` (eg1 mints, eg2 copies), `.tmp/eg-poc1-root-ca.crt` (demo 54) and `.tmp/eg-poc2-root-ca.crt` (demo 52) — not the poc clusters' `clustermesh-root-ca` | **Taken** — independence; a client trusts one file per lab it runs (three EG roots plus the Cilium lab's today), and each README says why |
...
| D11 | **`loadBalancerClass` is mandatory on every Envoy Gateway door, in every demo — including demo 51 where kube-vip is alone** | **Taken** (operator, 2026-09-18: *"for our envoy gateway api setup use cases I need to have loadBalancerClass — this best practice that we will get to use eventually and allow us to train application teams"*). The contract a team learns: the `EnvoyProxy` attached to their Gateway names the load balancer (`envoyService.loadBalancerClass: kube-vip.io/kube-vip-class` in 51, `metallb.io/metallb` in 52) beside the address; the load balancers run class-only (kube-vip `--lbClassOnly` + the cloud-provider's `KUBEVIP_ENABLE_LOADBALANCERCLASS=true`, MetalLB `--lb-class`), so a class-less `type: LoadBalancer` Service stays `<pending>` — measured in phase 0 (R0.4) — and the check has a row proving it. Cilium's LB IPAM has the same knob (`loadBalancerClass: io.cilium/lb-ipam`) — `docs/EG-VS-CILIUM.md` gets the row |
...
- **Three roots of trust** (D8): `.tmp/eg-root-ca.crt` (eg1/eg2),
  `.tmp/eg-poc1-root-ca.crt`, `.tmp/eg-poc2-root-ca.crt`; a reader trusts one
  file per lab; `docs/EG-VS-CILIUM.md` states it.
