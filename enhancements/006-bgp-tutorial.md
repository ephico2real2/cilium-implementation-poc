...
### 3.3 The dashboard in the fabric (`dashboard/` in [ephico2real2/bgp-fabric](https://github.com/ephico2real2/bgp-fabric))
...
| 1 — the fabric and the dashboard | **46** (Colima) and **55** (plain Docker) | The four routers in compose, converged, the source's exercises re-cast (edge ↔ spine ↔ leaves), the `frr-agent` image and the dashboard image built and pushed to `ghcr.io/ephico2real2/`, the dashboard deployed in poc1 and served as `bgp.poc.local`, reading the four routers over the fabric; node routes applied; no Cilium BGP yet | `demos/55-bgp-fabric-desktop/{fabric/compose.yaml,fabric/frr/*/frr.conf,fabric/frr/*/daemons,frr-agent/,dashboard/,10-dashboard.yaml,20-route.yaml,apply.sh,check.sh,cleanup.sh,GUIDE.md}`, `scripts/fabric-up.sh`, `scripts/fabric-down.sh`, `scripts/fabric-node-routes.sh` | R1, R2, R7 |
...
| attach — kube-vip BGP on eg-poc1 (§9) | **56** | Migration of eg-poc1 from L2 (demo 54) to BGP: kube-vip AS 65021, peers `172.19.254.11:65101::false` / `172.19.254.12:65102::false` (no password on this kernel — sheet row 3; a real kernel carries the fabric's one MD5), election then active-active; doors `bgp-http-gw` `10.98.0.10` and `bgp-grpc-gw` `10.98.0.11`; ETP Local measured then Cluster; gRPC matrix from `client0`; demo 54's `.100`/`.101` stop answering (`vip_arp=false`); cleanup restores `clusters/eg/kube-vip-ds.yaml` | `demos/56-kube-vip-bgp/{10a,10b,20,20a,40,50,apply.sh,check.sh,cleanup.sh}` | §9 D15–D19 |
...
| D13 | Where the fabric and dashboard code live: the fabric under `demos/46-bgp-fabric-colima/` and `demos/55-bgp-fabric-desktop/`, the dashboard and agent in their own repository (`ephico2real2/bgp-fabric`, pinned by `scripts/bgp-fabric.env`); originally this repo under `demos/46-bgp-fabric/` (configs, compose, Go sources), images on `ghcr.io/ephico2real2/{frr-agent,bgp-dashboard}` | **Taken — OPEN if the operator prefers a separate repository** (the lab's forks live in their own repos; this is not a fork) |
...
**the fabric is built first as a standalone lab that any cluster lab attaches to** — demo 46 on Colima, demo 55 on a plain Docker engine, each numbered below the demos that peer into it, and the first two labs to
...
- **Demo 56 — kube-vip in BGP mode on `eg-poc1`.** A **migration**, not a second announcer beside L2: kube-vip
  ...
  `bgp_peers=172.19.254.11:65101::false,172.19.254.12:65102::false` (no password: this VM's kernel refuses
  `TCP_MD5SIG` and gobgp then never connects — sheet row 3; a real kernel carries the fabric's one MD5),
  ...
  `client0` through edge → spine → leaf → node, ETP Local then Cluster, the gRPC matrix (the Mac route was
  absent — the client0 half is the record), a node paused → BGP withdrew at 13 s (hold 9 s + the 5 s tick),
  the node went `Ready=Unknown` at 43 s and its endpoints were pruned, yet 2 of the 4 probes after that still
  timed out: the Envoy Gateway controller (one replica) sat on the paused node, so the surviving Envoy kept
  the dead shopapi endpoint until unpause. A silent node needs BGP, the node grace period AND a live
  control plane for the door.
