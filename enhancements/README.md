...
| 003 | cf2cnp consumes Cilium's own policy spec: `github.com/cilium/cilium` `pkg/policy/api` types (291 schema paths vs 20 hand-written fields), `Sanitize()` on a copy + the embedded CRD of the same version as two validation layers, the supported spec printed and released; `fromCIDR`, `enableDefaultDeny`, deny lists, ICMP follow | 29 Part 10 (dashboard), 35 (regenerated policies validate) | [003-cf2cnp-cilium-policy-api.md](003-cf2cnp-cilium-policy-api.md); forensics on the fork: [`docs/CRD-SPEC-FORENSICS.md`](https://github.com/ephico2real2/cf2cnp/blob/enh/E12-cilium-policy-api/docs/CRD-SPEC-FORENSICS.md) | **released 0.7.0** (2026-09-13) — reviewed twice ([REVIEW_ENH-003](../docs/REVIEW_ENH-003.md)); 002's phase 0 unblocked |
