...
| 002 | The shop platform on the mesh: global services with local affinity, a gateway per cluster with pinned addresses, a database in one cluster with cluster-aware policies, egress IPs, load + HPA, failure and DR scenarios | — (issues after the plan is approved) | [002-shop-platform-clustermesh.md](002-shop-platform-clustermesh.md) | **plan for review** — five open decisions in its §5; egress gateway is documented as incompatible with Cluster Mesh, so that part goes on a throwaway cluster |
