...
| 3 | Phase 2: `.github/workflows/lab.yaml`, one job per demo group — `lab-route-b.yaml`'s tutorial step (SETUP 9.3b/9.5, demo 07 Parts 4–5 run as the guide prints them) is the seed (core, mesh, observability, policy-tools), the demo scripts under `scripts/record.sh`, the policy tests, Playwright captures as artefacts | the runner, task 2 | not started |
