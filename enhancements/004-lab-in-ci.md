...
| **the six chapters green, the pages as tests measured** (chapter 32 on the ingress side, demo 15's check before the cell, the Eureka wait) | run 34930321170: chapters 26/27/32/30/31/35 all green with every promised line (26 FORWARDED by the generated policy and DROPPED; 27 two policies Valid, pos 200, stranger and kiosk rc=1; 32 `1 rule(s) added`, idempotent, kiosk 200; 30 `/admin` 403 from pos and the frontend, the stranger rc=1; 31 example.com 200 by name, github.com rc=1; 35 six policies Valid, six 200s, three rc=1); the wait HTTP 1.39 req/s, 4,920 DNS queries, 1,102 dropped, 302,192 flows, 893 ICMP, 2,113 observer lines, **Tempo 200 / 200**; demo 15's in-cluster check 239 payments / 0 failed / poc1 73, poc2 166; the petclinic `200`, `George Franklin pets: ['Leo']`, `6 vets`, `201` after Eureka listed 4 applications UP (25 s); the report in 41 s; **17 of 18 pages met their expectations, both verdict-dashboard tests passed** (45 panels, only the allowed empties); the bank map expected `api` (shown only while a round runs exercise.sh — relaxed); `timeout` cannot run shell functions (two report sections empty — fixed) | [run 34930321170](https://github.com/ephico2real2/cilium-implementation-poc/actions/runs/34930321170), captures `ci-captures/2026-09-15T0531Z_lab-observability_run34930321170-1_two-clusters-meshed` |
| **demo 36 — one root, everywhere** (trust-manager's Bundle on both clusters, the runner's OS trust store at the root step, the pod's mount) — the first run green end to end with every test on | run 34933611546: `trust-manager v0.25.0, Bundle enterprise-root Synced` on both clusters at the bring-up; `1 added, 0 removed; done.` and `openssl verify -CApath /etc/ssl/certs: OK` on the runner; the routes step `issuer=CN = clustermesh-root-ca` on the wildcard and `curl by name with no --cacert → http 404, ssl_verify_result 0`; the report's demo 36 section: the ConfigMap in 24 of 24 namespaces (poc1) and 11 of 11 (poc2), one fingerprint, the forensic client `https://bank.poc.local` 200 with the mounted root and curl exit 60 without (Grafana 302 / 60); demo 15's step 7 through the Gateway `http 200` on the OS bundle; every report section 0 trouble words; the wait 942 ICMP, 2,151 observer lines, Tempo 200 / 200; 18 of 18 pages met their expectations | [run 34933611546](https://github.com/ephico2real2/cilium-implementation-poc/actions/runs/34933611546) |
| **demo 36 Part 3 — Kyverno v1.19.1, the mount without asking** (a MutatingPolicy on one label; the complete bundle; the four runtime variables) | run 34936560683: a webhook race at the Bundle apply on poc2 (`no route to host` right after `helm --wait`; gotcha #107); run 34937306210: Kyverno up, the policy ready, the labelled pod mutated as designed but its flagless curl `ssl_verify_result 20` — the official curl image pins `CURL_CA_BUNDLE`, which curl reads before `SSL_CERT_FILE` (read from curl/curl-container and curl's `tool_operate.c`); **run 34939744189, green end to end**: the bundle 151 certificates (the public roots + ours) on both clusters, the labelled pod's admitted spec `env=SSL_CERT_FILE CURL_CA_BUNDLE REQUESTS_CA_BUNDLE NODE_EXTRA_CA_CERTS`, `curl https://bank.poc.local/` from it with no flag **http 200, ssl_verify_result 0**, the ConfigMap in 26 of 26 namespaces, every page's expectations met | [run 34939744189](https://github.com/ephico2real2/cilium-implementation-poc/actions/runs/34939744189) |
