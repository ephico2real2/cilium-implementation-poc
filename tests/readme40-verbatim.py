...
non-blank lines contains 'recorded' (any case). The shopctl load tables
against a local HTTP server are unmarked and skipped. Empty lines inside
a fence are ignored. Comparison is rstrip, so trailing spaces on either
side do not fail.
"""
import re, sys
readme = open('demos/40-shop-mesh-phase0/README.md').read()
tx = {l.rstrip() for l in open('demos/40-shop-mesh-phase0/output/transcript.txt')}
