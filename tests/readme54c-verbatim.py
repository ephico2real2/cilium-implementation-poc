#!/usr/bin/env python3
"""every line inside a recorded ```text block of demo 54c's README must
exist verbatim in output/transcript.txt (record.sh's rule).

Scope: only the LAST apply in the transcript (from the last
"— demo 54c apply" header to the end). The one exception is the
"Runs that did not go to plan" section, matched against the whole
transcript.
"""
import re, sys

readme = open('demos/54-eg-poc1-kube-vip-colima/README.md').read()
lines = open('demos/54-eg-poc1-kube-vip-colima/output/transcript.txt').read().split('\n')
starts = [i for i, l in enumerate(lines) if l.rstrip().endswith('— demo 54c apply')]
