...
for leaf in ("leaf1", "leaf2"):
    ...
    if "bgp listen range 172.20.0.0/17 peer-group SERVERS" not in config:
        bad.append("%s lacks Colima listen 172.20.0.0/17" % leaf)
    if "bgp listen range 172.18.0.0/17" in config or "bgp listen range 172.19.0.0/17" in config:
        bad.append("%s still listens on a Desktop /17" % leaf)
...
# the EG overlay is this family's (P4); no Cilium overlay on Colima
overlay = root / "fabric/compose.lan-eg.yaml"
if not overlay.is_file():
    bad.append("compose.lan-eg.yaml missing — leaves attach at 172.20.254.11/.12")
else:
    ov = overlay.read_text()
    if "172.20.254.11" not in ov or "172.20.254.12" not in ov:
        bad.append("compose.lan-eg.yaml does not pin leaves at 172.20.254.11/.12")
    if "172.19.254." in ov:
        bad.append("compose.lan-eg.yaml still uses the Desktop node-LAN .254")
