...
| 3 | Session security: one password per fabric, TTL | the same `FABRIC_BGP_PASSWORD` (`lab-bgp`); kube-vip on eg-poc1 sends **no password** (`bgp_peers=172.19.254.11:65101::false,172.19.254.12:65102::false`): gobgp sets `TCP_MD5SIG` on its connecting socket and this VM's kernel refuses it, so a password keeps the session in ACTIVE forever (measured 2026-09-20; without it, Established in 25 s) — on a real kernel the speaker signs; TTL 1 (gobgp `ttl = 1` for eBGP, no GTSM) | `neighbor SERVERS password ${FABRIC_BGP_PASSWORD}`; no GTSM on SERVERS; demo 56 header on `10a`/`10b` |
