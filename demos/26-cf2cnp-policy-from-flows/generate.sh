...
POST=(curl --silent --show-error --fail-with-body --cacert "${ROOT_CA:-docs/root-ca.crt}" --resolve "cf2cnp.poc.local:443:$GW" -X POST "https://cf2cnp.poc.local/generate${QUERY:+?$QUERY}" -H 'Content-Type: application/json' --data-binary "@$F")
