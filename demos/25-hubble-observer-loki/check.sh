...
# ROOT_CA: the root the Gateway's certificate chains to — docs/root-ca.crt is the laptop's; a fresh lab (the runner) exports its
# own and passes it here, or the Grafana and cf2cnp lines below fail TLS verification and print a JSON error (run 34891888521)
...
curl -s --cacert "${ROOT_CA:-docs/root-ca.crt}" --resolve grafana.poc.local:443:$GW -u admin:poc-grafana https://grafana.poc.local/api/datasources | python3 -c 'import json,sys; [print("  ds:", d["name"], d["type"], d["url"]) for d in json.load(sys.stdin) if d["type"]=="loki"]'
curl -s --cacert "${ROOT_CA:-docs/root-ca.crt}" --resolve grafana.poc.local:443:$GW -u admin:poc-grafana "https://grafana.poc.local/api/search?query=Hubble%20Observer" | python3 -c 'import json,sys; [print("  dashboard:", d["title"], "| folder:", d.get("folderTitle"), "| /d/"+d["uid"]) for d in json.load(sys.stdin)]'
...
echo "  health: $(curl -s --cacert "${ROOT_CA:-docs/root-ca.crt}" --resolve cf2cnp.poc.local:443:$GW https://cf2cnp.poc.local/health)"
