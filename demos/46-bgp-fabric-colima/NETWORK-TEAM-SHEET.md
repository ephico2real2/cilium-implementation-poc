...
`CONFIG_TCP_MD5SIG=y`). Attaching a kind cluster is the next phase
(one kind cluster per Docker context).

## Fabric (common; this phase is the fabric alone)
...
| 3 | Session security: one password per fabric, TTL | `FABRIC_BGP_PASSWORD` in [`fabric/.env`](fabric/.env) (copy [`.env.example`](fabric/.env.example); default `lab-bgp`) on every session; TTL 1 (same segment), **no GTSM** on SERVERS — kube-vip (gobgp) and FRR-K8s send TTL 1 and cannot pass `ttl-security` | `neighbor … password ${FABRIC_BGP_PASSWORD}` rendered at start; committed `frr.conf` has no secret. **This VM signs the sessions.** Apply `2026-09-20T23:09:15Z`: kernel `6.8.0-117-generic`, `CONFIG_TCP_MD5SIG=y`; 20 packets captured on leaf1 each carrying a TCP-MD5 option; check counted `md5-option packets=18`; wrong password on leaf1 neighbor `10.200.1.3`: `Established→Idle; restored Established`; while healthy `TcpExtTCPMD5{NotFound,Unexpected,Failure}` all 0. An unsigned session also shows zero kernel failures — the proof is the wire count **and** the mismatch. check.sh rows 12–14 record it |
...
## Envoy Gateway lab (`kind-eg`) — next phase

Not attached on this Docker context. `kind-eg` lives on Docker
Desktop. The listen range and the EG prefix-lists are already on
both leaves. Creating a kind cluster inside the Colima VM is the
next phase (one kind cluster per Docker context).
...
| 8 | Route back to the servers' networks | `kind-eg` `172.19.0.0/16` is connected on both leaves when a Colima-side overlay exists | connected routes only — not this phase |
| 10 | Hand-off record | this file; password in `fabric/.env` (from `.env.example`) | — |

## Cilium lab (`kind`) — next phase

Not attached. The Cilium prefix-lists are already on both leaves.
...
| 8 | Route back to the servers' networks | `kind` `172.18.0.0/16` is connected on both leaves when a Colima-side overlay exists | connected routes only — not this phase |
| 10 | Hand-off record | this file; password in `fabric/.env` (from `.env.example`) | — |
