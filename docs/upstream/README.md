...
|---|---|---|---|
| Hubble's `destination_workload` empty for Gateway traffic to a remote backend | comment on the existing bug (posted); **a review comment on the upstream fix PR** (drafted) | [hubble-l7-dashboard.md](hubble-l7-dashboard.md); [drafts/cilium-48563-review-comment.md](drafts/cilium-48563-review-comment.md) | posted 2026-09-17 on cilium/cilium#25676; the lab then **fixed it in a fork and measured the fix** (demo 39: `ephico2real2/cilium` branch `hubble/remote-workload-via-cep`, three commits, both clusters on the image) — and found upstream's open draft #48563 doing the same with CES and the kvstore path; the review comment bringing it the two things it lacks (the CRD schema-version bump; the L7/Gateway reproduction) **posted 2026-09-18 on the operator's word**: [cilium/cilium#48563 (comment)](https://github.com/cilium/cilium/pull/48563#issuecomment-5725111947) |
