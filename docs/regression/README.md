...
|---|---|---|---|
| **1. The quick check** — `scripts/lab-regression.sh` | fourteen questions in plain English, each answered PASS or FAIL with the number that decided it | about two minutes | after any change, before saying "done" |
...
| **3. The whole lab from nothing** — the `lab-observability` GitHub Action | a fresh runner builds both clusters, installs every demo, generates traffic, generates and applies policies, checks every page, screenshots every dashboard | about 55 minutes | on every branch that changes a version pin |
...
The command, exactly as the CI runs it:

```sh
cilium connectivity test --context kind-poc1 --multi-cluster kind-poc2 --test '!no-unexpected-packet-drops'
```

(The `!no-unexpected-packet-drops` part skips one test that counts every dropped packet — on this lab there are
demos whose whole point is to drop packets, so that test would always complain.)
...
the Cilium logs of every pod and complains if any line matches its list of worrying messages. It found 16, of four
kinds. Every one is explained by what we did that evening:

| Message (what the log said) | When | What it means, plainly | Regression? |
|---|---|---|---|
| `Forcefully terminating sockets connected to deleted service backends not supported by underlying kernel` (8 times) | 22:26–22:27 and 22:33–22:34 — the minutes the Cilium pods were being replaced on each cluster | When a pod behind a service goes away, Cilium can cut the open connections to it on the spot — but only on a kernel that has that feature. The Docker Desktop kernel (`7.0.12-linuxkit`) does not, so Cilium says so once per rollout and lets the connections time out on their own. Same before and after the upgrade | No — a fact about this Mac's kernel, logged during the rollout |
| `Error while getting cilium-health status … connection refused` (4 times, "6 occurrences") | 22:27 and 22:34, seconds after each new agent started | The health-checker inside the agent was asked before it had finished starting | No — startup, during the rollout |
| `Failed to retrieve flows from peer … Unavailable` (hubble-relay, 2 times) | 22:33:41 | The relay lost its connection to an agent that was being replaced, and reconnected | No — the rollout |
| `Envoy: Discarded invalid access log message … string field contains invalid UTF-8` (2 times, "225 occurrences") | 00:23:53 — *during* the test | The test sends deliberately odd requests through the L7 proxy; Envoy's access log could not encode 225 of them and dropped the log lines (the requests themselves were handled). This is the one message caused by the test, not by us | No — the test's own traffic; worth remembering if it ever shows up outside a test |
...
**How to read a run of your own:** look for the line starting `✅ All 87 tests` or `❌ N/87 tests failed`. If N is
...
connectivity test too (an optional input) and the GitHub runner itself died 70 minutes in — "The hosted runner lost
communication with the server", which GitHub explains as the runner starved of CPU or memory. That is the *runner*
...
|---|---|---|
| 1 | Both clusters run the expected Cilium version | Did the upgrade actually land? (We learned the hard way — gotcha #117 — that Helm can say "deployed" while nothing changed.) |
...
| 6 | The load-balancer addresses have an owner | Is some node answering for each of the lab's public IPs? |
...
| 12 | The tutorial dashboards' queries return data | Do the 30 tutorial panels all have data (none says "No data")? |
...
Run it: `scripts/lab-regression.sh` (about two minutes; the table is also saved under `output/regression/`). It exits
...
```text
== lab regression 2026-09-18T01:05:07Z — contexts kind-poc1 kind-poc2 — expected Cilium 1.20.2
  ...
  PASS   Every Cilium agent is healthy                                                            poc1 Cilium=OK Envoy=OK; poc2 Cilium=OK Envoy=OK cilium status --wait exits 0 on both
  ...
  PASS   The Gateway answers on every published address                                           7/7 200                                    all 7 URLs return 200 (bank may be 401)
  ...
  PASS   Hubble metrics arrive from both clusters                                                 poc1 257.2/s, poc2 82.5/s                  rate > 0 for poc1 and poc2
  PASS   Network policy is still enforcing (drops exist) and traffic still flows (forwards exist) FORWARDED 250.0/s, DROPPED 2.6/s           FORWARDED > 0 and DROPPED > 0 on poc1
  PASS   The flow observer is streaming to Loki                                                   ready 1/1, 320 lines in 2 min              ready 1/1 and log lines in the last 2 min > 0
...
summary: 14 PASS, 0 FAIL, 0 WARN — saved to output/regression/20260918T010507Z.txt
```

Read it top to bottom: the version landed on both clusters (2 of 2 agents each), Cilium says OK, the mesh is up from
both sides, all seven public names answer 200, all seven Gateway listeners are programmed, the four public addresses
have an owner, monitoring sees 255 flows a second from poc1 and 82 from poc2, policy is dropping 2.6 a second while
248 a second get through, the observer wrote 318 lines in two minutes, cf2cnp is 0.9.0 and healthy, the nine
dashboards exist, the tutorial's 30 panels all have data, demo 37's two doors are up at their addresses, and the
connectivity test's only failure was the log scan explained above.
...
captures show four dashboards the rows touch — the observer dashboard, *Hubble Metrics per cluster*, the app-keyed L7
dashboard (the workaround for cilium/cilium#25676) and the tutorial's Cilium page.
