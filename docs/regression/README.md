...
| [35296914935](https://github.com/ephico2real2/cilium-implementation-poc/actions/runs/35296914935) — with a wait for Loki | the wait step | the check **13 PASS, 0 FAIL, 1 WARN** again; the wait ran its full six minutes and Loki still had 0 observer lines | not timing after all: the trimmed stack had skipped `tempo` and `collectors`, and the OTel collector *is* the Loki shipper (the lab's own table says so). The observer wrote; nothing carried it |
